...
function buildOtpEmail(code: string, purpose: 'signup' | 'password-reset' = 'signup'): string {
  const description =
    purpose === 'signup'
...
export const sendOtp = onCall(
  ...
  async (request) => {
    ...
    if (existing.exists) {
      ...
      const ageSeconds =
        (Date.now() - (d.createdAt as admin.firestore.Timestamp).toMillis()) / 1000;
...
export const verifyOtp = onCall(async (request) => {
  ...
  if (!email || !code) {
    throw new HttpsError('invalid-argument', 'Email and OTP code are required.');
  }
  ...
  if (!snap.exists) {
    throw new HttpsError('not-found', 'No OTP found for this email. Request a new code.');
  }
...
export const sendSignupOtp = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      if (e instanceof HttpsError) throw e; // rethrow our own error
    ...
    try {
      ...
      throw new HttpsError('internal', `Firestore write failed: ${e.message}`);
    }
    ...
    if (error) {
      ...
      throw new HttpsError('internal', `Failed to send email: ${JSON.stringify(error)}`);
    }
...
export const verifySignupOtp = onCall(async (request) => {
  ...
  const nationalId: string = (request.data.nationalId ?? '').trim();

  console.log('[verifySignupOtp] called for:', email);

  if (!email || !code || !password || !fullName) {
    console.error('[verifySignupOtp] missing fields — email:', !!email,
      'code:', !!code, 'password:', !!password, 'fullName:', !!fullName);
  ...
  const d = snap.data()!;
  console.log('[verifySignupOtp] OTP record — used:', d.used,
    'attempts:', d.attempts, 'expired:', Date.now() > d.expiresAt.toMillis());
  ...
  try {
    ...
    throw new HttpsError('internal', `Failed to create account: ${e.message}`);
  }
  ...
  try {
    ...
    throw new HttpsError('internal', `Failed to save user data: ${e.message}`);
  }
